...
export default function Sidebar() {
    ...
    return (
        <div className="w-80 bg-base-100 h-full text-base-content flex flex-col shadow-inner">
            {/* 상단: 도감 목록 영역 (스크롤 가능) */}
            <div className="p-4 flex-1 overflow-y-auto space-y-4">
                
                {/* --- 적 정보 섹션 --- */}
                <div className="collapse collapse-plus bg-base-200 shadow-sm rounded-xl">
                    ...
                    <div className="collapse-content bg-base-200 pt-4">
                        ...
                        <div className="space-y-2">
                            {filteredEnemies.map((enemy) => (
                                <div key={enemy.enemy_id} className="collapse collapse-arrow bg-base-200 border border-base-300 shadow-sm rounded-lg">
                                    <input type="checkbox" className="peer" />
                                    <div className="collapse-title text-sm font-bold flex justify-between items-center">
                                        <span>{enemy.name}</span>
                                        <span className="text-error text-xs">난이도 {enemy.base_difficulty}</span>
                                    </div>
                                    <div className="collapse-content text-xs text-base-content/90 border-t bg-base-200">
                ...
                <div className="collapse collapse-plus bg-base-200 shadow-sm rounded-xl">
                    ...
                    <div className="collapse-content bg-base-200 pt-4">
                        ...
                        <div className="space-y-2">
                            {filteredNPC.map((npc) => (
                                <div key={npc.npc_id} className="collapse collapse-arrow bg-base-200 border border-base-300 shadow-sm rounded-lg">
                                    <input type="checkbox" className="peer" />
                                    <div className="collapse-title text-sm font-bold flex justify-between items-center">
                                        <span>{npc.name}</span>
                                        <span className="text-info text-xs">{npc.occupation}</span>
                                    </div>
                                    <div className="collapse-content text-xs text-base-content/90 border-t bg-base-200">
                    ...
                    <div className="collapse-content bg-base-200 pt-4">
                        ...
                        <div className="space-y-2">
                            {filteredItems.map((item) => (
                                <div key={item.item_id} className="collapse collapse-arrow bg-base-200 border border-base-300 shadow-sm rounded-lg">
                                    <input type="checkbox" className="peer" />
                                    <div className="collapse-title text-sm font-bold flex justify-between items-center">
                                        <span>{item.name}</span>
                                        <span className="text-warning text-xs">{item.grade}</span>
                                    </div>
                                    <div className="collapse-content text-xs text-base-content/90 border-t bg-base-200">
                                        <div className="py-2">
                                            <p className="text-info font-bold mb-1">{item.base_price} Gold</p>
                                            <p className="leading-relaxed">{item.description}</p>
                                            {item.effect_value && <p className="text-error mt-1">{item.effect_value}</p>}
                                        </div>
                                    </div>
                                </div>
                            ))}
